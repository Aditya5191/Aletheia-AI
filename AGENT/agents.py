...
from llm import get_llm, get_creative_llm, get_fallback_llm, get_profiler_llm
from tools import SandboxTool, LocalPythonTool, MCPAlgorithmTool, PDFReportTool

# Tools instances
sandbox_tool = SandboxTool()
local_tool = LocalPythonTool()
...
data_profiler_agent = Agent(
    ...
    goal="""
        ...
        Try running in the sandbox first (run_python_in_sandbox). 
        If it fails due to Docker issues, USE run_python_locally as a fallback.
        
        The profiling code must:
        - Load /sandbox/dataset.csv (or the provided local path)
    ...
    backstory="""
        ...
        and what statistical patterns matter for bias analysis.
    """,
    tools=[sandbox_tool, local_tool],
    llm=get_profiler_llm(),
...
bias_analyst_agent = Agent(
    ...
    goal="""
        ...
        Step 2 — Code Generation + Execution:
          ...
          Try run_python_in_sandbox first. If Docker fails, use run_python_locally.
        ...
        Step 3 — Retry on Failure:
          ...
          and call the tool again with attempt=2 (then attempt=3 max).
    ...
    backstory="""
        ...
        translate it directly to Python. When code fails you read stderr precisely and 
        fix the exact error.
    """,
    tools=[mcp_tool, sandbox_tool, local_tool],
    llm=get_llm(),
    fallback_llm=fallback_llm,
    verbose=True,
    allow_delegation=False,
