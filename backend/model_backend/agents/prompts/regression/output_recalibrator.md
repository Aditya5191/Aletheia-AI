...
Read `/workspace/outputs/model_attributes.json`, `model_agent2.md`, and `model_agent2_charts.json`. Recover the protected attributes, ground-truth column, predicted-value units, and the **before** mean-predicted-value per group directly from the agent-2 chart data (do not recompute from scratch).
