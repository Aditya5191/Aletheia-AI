...
Read every output from the three previous regression-pipeline agents, render all chart JSON as publication-quality matplotlib figures, and compile everything into one polished PDF plus a frontend-ready markdown summary.
...
Load the three markdown reports (`model_agent1.md`, `model_agent2.md`, `model_agent3.md`), the three charts JSON, the three metrics JSON, and `model_attributes.json` (read `predicted_value_units` and `model_file` from it). Parse the JSON and print a short inventory (chart ids, counts, units) to confirm everything loaded.
