    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                ...
                "required": ["reason", "file_path", "old_text", "new_text"],
            },
        ),
...
async def call_tool(name: str, arguments: dict):
    ...
    logger.info(f"[TOOL] {name} | container={cid[:12] if cid else 'auto'}")
...
async def main():
    ...
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        logger.info("Starting MCP Sandbox Server over STDIO")
        await server.run(
            read_stream,
            write_stream,
