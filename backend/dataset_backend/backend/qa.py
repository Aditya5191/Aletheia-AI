...
def get_available_agent_context() -> dict:
...
def build_qa_prompt(question: str, agent_numbers: list[int], context: dict) -> list:
    ...
    system_text = (
        "You are the fairness-audit assistant embedded in the Aletheia AI pipeline. "
...
async def stream_answer(question: str, agent_numbers: list[int]) -> AsyncIterator[str]:
    context = get_available_agent_context()
    messages = build_qa_prompt(question, agent_numbers, context)
